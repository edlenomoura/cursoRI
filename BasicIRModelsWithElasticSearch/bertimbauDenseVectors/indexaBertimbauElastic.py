...
def gerar_embedding_documento(texto, tokenizer, model, device):
    ...
    Args:
        ...
        model: Modelo BERT
        device: 'cuda' ou 'cpu'
        CUDA é uma plataforma de computação 
        paralela e modelo de programação criada 
        pela NVIDIA. O CUDA ajuda os desenvolvedores 
        a acelerar suas aplicações aproveitando 
        hardware especializado de GPUs para realizar 
        cálculos paralelos de forma eficiente.
    Returns:
        numpy.array: Embedding de 768 dimensões
    """
    ...
    configuracao = {
        ...
        "mappings": {
            "properties": {
                ...
                "embedding_title": {
                    "type": "dense_vector", # campos que terão vetores densos, pode ser com qq vetorizador, não só BERT*
                    "dims": 768,  # Dimensão do BERTimbau
                    "index": True,  # Habilita busca kNN
                    "similarity": "cosine"  # Similaridade cosseno é a padrão para comparar vetores densos
                },
                "embedding_highlight": {
                    "type": "dense_vector",
                    "dims": 768,
...
def indexar_com_bertimbau(es, documentos, tokenizer, model, device, nome_indice='ementas_bertimbau'):
    ...
    for doc in tqdm(documentos, desc="Processando"):
        ...
        # Ação de indexação
        # AQUI MONTA UM DOCUMENTO COM SEUS VETORES PARA INSERIR NO INDICE
        acao = {
            "_index": nome_indice,
            "_id": doc.get('id'),
            "_source": {
